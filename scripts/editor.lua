...
local REFERENCE_WIDTH = 2560
local REFERENCE_HEIGHT = 1440

local UI_SCALE = math.min(
    engine.screen_width() / REFERENCE_WIDTH,
    engine.screen_height() / REFERENCE_HEIGHT
)

local function scale(v)
    return math.floor(v * UI_SCALE + 0.5)
end

local function font(v)
    return math.max(12, math.floor(v * UI_SCALE + 0.5))
end
...
local spell_sidebar_root = {
    x = math.floor(engine.screen_width() * 0.10),
    y = scale(240)
}
...
local function draw_spell_slot_buttons()
    ...
    local height = scale(46)
    local spacing = scale(12)

    engine.text(
        SpellConfig.slot_title,
        font(26),
        spell_sidebar_x(0),
        spell_sidebar_y(0),
        width,
    ...
    if engine.button(
        "add_spell",
        spell_sidebar_x(x),
        spell_sidebar_y(y + (SpellConfig.get_spell_count() * (height + spacing)) + scale(12)),
        width,
        height,
        "Add Spell"
    ) then
...
local function draw_spell_editor()
    if SpellConfig.get_spell_count() == 0 then
        ...
        engine.text(
            "Create a spell to begin editing its attributes.",
            font(28),
            0,
            editor_y(scale(120)),
            engine.screen_width(),
    ...
    local slider_width = scale(360)
    local controls_x = (engine.screen_width() - slider_width) * 0.5

    local name_y = scale(150)
    local top_y = scale(220)
    ...
    engine.text(
        "Slot " .. tostring(SpellConfig.active_slot) ..
        " / " .. tostring(SpellConfig.get_spell_count()) ..
        " | " ..
        SpellConfig.get_slot_range_hint(),
        font(20),
        0,
        editor_y(scale(75)),
        engine.screen_width(),
    ...
    local entity_label =
        spell.entity_id and
        ("Entity #" .. tostring(spell.entity_id))
        or "Entity pending"
    ...
    engine.text(
        ...
        font(22),
        controls_x,
        editor_y(top_y + (row * row_step) - scale(10)),
        slider_width,
        scale(24)
    )
    ...
    spell.color = spell.color or {
        r = 1.0,
        g = 0.82,
        b = 0.33
    }

    local swatch_size = scale(30)
    ...
    spell.color.r = engine.slider(
        ...
        controls_x,
        editor_y(top_y + (row * row_step)),
        slider_width - swatch_size - scale(20),
        "R"
    )
    ...
    engine.text(
        "Calculated Damage: " .. tostring(spell.damage),
        font(30),
        panel_x,
        editor_y(summary_y + scale(34)),
        panel_width,
        scale(30)
    )
end
